...
def _parse(url):
    ...
    print("[CCcamInfo]1 parsed=%s scheme=%s path=%s host=%s port=%s" %
          (parsed, scheme, pathz, host, port))
...
config.cccamlineedit.protocol = NoSave(
    ConfigSelection(
        default="C:", choices=[
            ("C:", _("CCcam")), ("N:", _("NewCamd"))]))
...
config.cccaminfo.blacklist = ConfigText(
    default="/etc/enigma2/CCcamInfo.blacklisted",
    fixed_size=False)
config.cccaminfo.profiles = ConfigText(
    default="/etc/enigma2/CCcamInfo.profiles",
    fixed_size=False)
        ...
        if fileExists(png):
            res.append(
                MultiContentEntryPixmapAlphaBlend(
                    pos=(
                        10, 7), size=(
                        67, 48), png=LoadPixmap(png)))
        res.append(
            MultiContentEntryText(
                pos=(
                    90, 8), size=(
                    900, 40), font=1, text=name))
    else:
        if fileExists(png):
            res.append(
                MultiContentEntryPixmapAlphaBlend(
                    pos=(
                        0, 0), size=(
                        35, 25), png=LoadPixmap(png)))
        res.append(
            MultiContentEntryText(
                pos=(
                    40, 3), size=(
                    500, 25), font=0, text=name))
    return res
...
def CCcamShareViewListEntry(
    ...
    if screenwidth and screenwidth == 1920:
        res = [
            (caidprovider, providername, numberofcards), MultiContentEntryText(
                pos=(
                    10, 5), size=(
                    800, 35), font=1, text=providername), MultiContentEntryText(
                pos=(
                    1050, 5), size=(
                    50, 35), font=1, text=numberofcards, flags=RT_HALIGN_RIGHT), MultiContentEntryText(
                pos=(
                    1100, 5), size=(
                    50, 35), font=1, text=numberofreshare, flags=RT_HALIGN_RIGHT)]
    else:
        res = [
            (caidprovider, providername, numberofcards), MultiContentEntryText(
                pos=(
                    0, 0), size=(
                    430, 20), font=0, text=providername), MultiContentEntryText(
                pos=(
                    430, 0), size=(
                    50, 20), font=0, text=numberofcards, flags=RT_HALIGN_RIGHT), MultiContentEntryText(
                pos=(
                    480, 0), size=(
                    50, 20), font=0, text=numberofreshare, flags=RT_HALIGN_RIGHT)]
    return res
...
class CCcamInfoMain(Screen):
    ...
    def keyNumberGlobal(self, idx):
        if self.working is False and (idx < len(self.menu_list)):
            ...
            if sel == _("General"):
                ...
                getPage(
                    self.url + "/clients",
                    self.showCCcamClients,
                    self.getWebpageError)

            elif sel == _("Active clients"):
                getPage(
                    self.url + "/activeclients",
                    self.showCCcamClients,
                    self.getWebpageError)
                ...
                getPage(
                    self.url + "/servers",
                    self.showCCcamServers,
                    self.getWebpageError)

            elif sel == _("Shares"):
                getPage(
                    self.url + "/shares",
                    self.showCCcamShares,
                    self.getWebpageError)

            elif sel == _("Share View"):
                self.session.openWithCallback(
                    self.workingFinished, CCcamShareViewMenu, self.url)

            elif sel == _("Extended Shares"):
                self.session.openWithCallback(
                    self.workingFinished, CCcamInfoShareInfo, "None", self.url)

            elif sel == _("Providers"):
                getPage(
                    self.url + "/providers",
                    self.showCCcamProviders,
                    self.getWebpageError)

            elif sel == _("Entitlements"):
                getPage(
                    self.url + "/entitlements",
                    self.showCCcamEntitlements,
                    self.getWebpageError)

            elif sel == _("ecm.info"):
                self.session.openWithCallback(
                    self.showEcmInfoFile, CCcamInfoEcmInfoSelection)

            elif sel == _("Menu config"):
                self.session.openWithCallback(
                    self.updateMenuList, CCcamInfoMenuConfig)
                ...
                self.session.openWithCallback(
                    self.profileSelected, CCcamInfoRemoteBoxMenu)

            elif sel == _("CCcam Prio Maker"):
                self.session.openWithCallback(
                    self.workingFinished, CCcamPrioMaker.Ccprio_Setup)

            elif sel == _("CCcam Organizer"):
                self.session.openWithCallback(
                    self.workingFinished, CCcamOrganizer.OrganizerMenu)
                ...
                self.session.openWithCallback(
                    self.workingFinished, CCcamInfoConfigSwitcher)
    ...
    def showCfgSelection(self):
        ...
        if lines:
            ...
            for line in lines:
                ...
                lineDescription = "%s %s %s" % (
                    lineElements[0], lineElements[1], lineElements[2])
                cfgLines.append((lineDescription, line))
            cfgLines.append((_("Add new CCcam line"), "newC"))
            cfgLines.append((_("Add new NewCamd line"), "newN"))
            self.session.openWithCallback(self.showCfgSelectionCallback, MessageBox, _(
                "Please select a line to edit or select add to create new line."), list=cfgLines)
    ...
    def showCfgSelectionCallback(self, line):
        if line:
            self.session.openWithCallback(
                self.workingFinished, CCcamLineEdit, line)
        else:
            self.workingFinished()
    ...
    def showCCcamGeneral2(self, html):
        ...
        if version_start_idx != -1:
            ...
            if version_end_idx != -1:
                ...
                self.infoToShow = "%s%s\n%s" % (
                    _("Version: "), version, self.infoToShow)
                    ...
                    if len(fields) > 5:
                        ...
                        if caid:
                            ...
                            providers_list.append(
                                _("CaID: ") + caid + " - " + _("Provider: ") + provider)
                ...
                if len(parts) > 7:
                    ...
                    if hostname:
                        ...
                        if self.uphop == -1:
                            maxdown = down.strip()
                            numberofcards = 1
                            providername = self.providers.get(
                                caidprovider, 'Multiple Providers given')
                            ...
                            if caidprovider not in providerList:
                                ...
                                shareList.append(
                                    CCcamShareViewListEntry(
                                        ...
                                        str(numberofreshare)))
                                self.list.append(
                                    [caidprovider, providername, numberofcards, numberofreshare])
                                totalproviders += 1
                            else:
                                ...
                                providername = self.providers.get(
                                    caidprovider, 'Multiple Providers given')
                                shareList[index] = CCcamShareViewListEntry(
                                    caidprovider, providername, str(numberofcards), str(numberofreshare))
                            ...
                            if int(up.strip()) == self.uphop:
                                ...
                                if caidprovider not in providerList:
                                    ...
                                    countList.append(1)
                                    reshareList.append(
                                        1 if int(down) > 0 else 0)
                                    shareList.append(CCcamShareViewListEntry(
                                        caidprovider, providername, '1', str(reshareList[-1])))
                                    self.list.append(
                                        [caidprovider, providername, '1', str(reshareList[-1])])
                                    totalproviders += 1
                                else:
...
class CCcamInfoProfileSetup(Setup):
    ...
    def okClicked(self):
        self.close(
            CCcamInfoRemoteBox(
                config.cccaminfo.name.value,
                config.cccaminfo.ip.value,
                config.cccaminfo.username.value,
                config.cccaminfo.password.value,
                config.cccaminfo.port.value))
                ...
                if len(tmp) == 5:
                    ...
                    try:
                        ...
                        if 1 <= port <= 65535:  # Ensure port is in a valid range
                            ...
                            self.profiles.append(CCcamInfoRemoteBox(
                                name, ip, username, password, port))
                        else:
                            print(
                                "Invalid port number {port} in profile:", name)
                    except ValueError:
    ...
    def saveConfigs(self):
        ...
        try:
            with open(config.cccaminfo.profiles.value, "w") as f:
                # Strip to remove any leading/trailing newlines
                f.write(content.strip())

        except IOError as e:
            self.session.open(
                MessageBox, _("Could not save the config file! Error: %s") %
...
class CCcamInfoShareInfo(Screen):
    ...
    def readShares(self):
        getPage(
            self.url + "/shares",
            self.readSharesCallback,
            self.readSharesError)
    ...
    def showContent(self):
        ...
        if fileName is not None and len(fileName) > 0:
            try:
                ...
                content = _("Could not open the file %s! Error: %s") % (
                    fileName[0], str(e))
            except Exception as e:
...
class CCcamInfoMenuConfig(Screen):
    ...
    def getBlacklistedMenuEntries(self):
        try:
            with open(config.cccaminfo.blacklist.value, "r") as f:
                content = f.read()
            if isinstance(content, bytes):
                # Decodifica per Python 2 se necessario
                content = content.decode("utf-8")
    ...
    def save(self):
        ...
        try:
            ...
            self.session.open(
                MessageBox,
                _("Configfile %s saved.") %
                config.cccaminfo.blacklist.value,
                MessageBox.TYPE_INFO)
        except IOError as e:
            # Log the error or handle it appropriately
            self.session.open(
                MessageBox, _("Could not save configfile %s! Error: %s") %
                (config.cccaminfo.blacklist.value, str(e)), MessageBox.TYPE_ERROR)
...
def fileReadLines(
    ...
    if debug or forceDebug:
        ...
        print("[%s] Line %d: %s %d lines from file '%s'." %
              (source, getframe(1).f_lineno, msg, length, filename))
    return lines
...
def fileWriteLines(filename, lines, source=DEFAULT_MODULE_NAME, debug=False):
    try:
        ...
        print(
            "[%s] Error %d: Unable to write %d lines to file '%s'!  (%s)" %
            (source, err.errno, len(lines), filename, err.strerror))
        msg = "Failed to write"
        result = 0
    if debug or forceDebug:
        print("[%s] Line %d: %s %d lines to file '%s'." %
              (source, getframe(1).f_lineno, msg, len(lines), filename))
    return result
