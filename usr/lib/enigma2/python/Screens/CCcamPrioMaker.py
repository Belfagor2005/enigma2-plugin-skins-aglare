...
class CCPrioMaker(Screen):
        ...
        if service:
            ...
            if info:
                if info.getInfo(iServiceInformation.sIsCrypted):
                    ...
                    if ssid:
                        self.ssid = "%04X" % (ssid)
                        self.Provider = info.getInfoString(
                            iServiceInformation.sProvider)
                        self.ServiceName = info.getName().replace(
                            '\xc2\x86', '').replace('\xc2\x87', '')
                        self.caidlist = info.getInfoObject(
                            iServiceInformation.sCAIDs) or None
                        if self.caidlist:
                            if config.plugins.ccprio.onecaid.value is False and len(
                                    self.caidlist) == 1:
                                self.ecmTimer.stop()
                            else:
                                self.parseEcmInfo()
    ...
    def parseEcmInfo(self, caidlist=0):
        ...
        if self.caid and len(
            self.caid) == 2 and len(
            self.caid[0]) > 0 and len(
                self.caid[1]) > 0 and self.caid[0] != "0000":
            caid = self.caid[0]
            provid = self.caid[1]
            ...
            if self.valssid not in PRIOLIST_P:
                ...
                for x in self.caidlist:
                    ...
                    if m == caid:
                        providval = provid
                    menu.append(
                        (_("Caid:") + str(m) + _("\tProvid:") + providval, str(m)))
                    tmplist.append(str(m))

                if caid not in tmplist:
                    menu.append(
                        ("Caid:" + str(caid) + "\tProvid:" + providval, str(caid)))
                ...
                if config.plugins.ccprio.askcaid.value is True:
                    ...
                    mess = _("Provider") + " :" + self.Provider + \
                        "  " + _("Service") + " :" + self.ServiceName

                    if config.plugins.ccprio.allcaid.value is False:
                        menu = []
                        menu.append(
                            ("Caid:" + str(caid) + "\tProvid:" + provid, str(caid)))
                        selection = 0
                        mess += _("\n All Caid:")
                        for x in tmplist:
                            mess += "%s " % (x)
    ...
    def parseEcmInfo_back(self, caidval=0):
        if caidval:
            ...
            provid = self.caid[1]
            if caid != "" and len(
                    caid) == 4 and caid != "0000" and self.valssid != "":
...
class Ccprio_Setup(Screen, ConfigListScreen):
    def __init__(self, session):
        ...
        list.append(
            getConfigListEntry(
                _("CCPrioMaker autostart"),
                config.plugins.ccprio.autostart))
        list.append(
            getConfigListEntry(
                _("Ask for the selection caid"),
                config.plugins.ccprio.askcaid))
        list.append(
            getConfigListEntry(
                _("Ask all caid"),
                config.plugins.ccprio.allcaid))
        list.append(
            getConfigListEntry(
                _("Editing one caid"),
                config.plugins.ccprio.onecaid))
        list.append(
            getConfigListEntry(
                _("Write-Debug"),
                config.plugins.ccprio.debug))
