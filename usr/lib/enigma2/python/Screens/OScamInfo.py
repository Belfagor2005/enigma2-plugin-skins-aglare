...
config.oscaminfo.username = ConfigText(
    default="username",
    fixed_size=False,
    visible_width=12)
config.oscaminfo.password = ConfigPassword(
    default="password", fixed_size=False)
...
def _ifinfo(sock, addr, ifname):
    ...
    if addr == 0x8927:
        return ':'.join(['%02x' % (char if six.PY3 else ord(char))
                        for char in info[18:24]]).upper()
    else:
...
class OscamInfo:
    ...
    def getUserData(self):
        ...
        if webif and port is not None:
            # oscam/ncam reports it got webif support and webif is running
            # (Port != 0)
                ...
                with open(conf, 'r') as data:
                    for i in data:
                        if "httpuser" in i.lower():
                            ...
                            # Once we encounter a httpallowed statement, we
                            # have to assume oscam/ncam webif is blocking us
    ...
    def openWebIF(self, part=None, reader=None):
        ...
        if self.port.startswith('+'):
            ...
            self.port.replace("+", "")
        if part is None:
            self.url = "%s://%s:%s/%sapi.html?part=status" % (
                self.proto, self.ip, self.port, NAMEBIN)
        else:
            self.url = "%s://%s:%s/%sapi.html?part=%s" % (
                self.proto, self.ip, self.port, NAMEBIN, part)
        if part is not None and reader is not None:
            self.url = "%s://%s:%s/%sapi.html?part=%s&label=%s" % (
                self.proto, self.ip, self.port, NAMEBIN, part, quote_plus(reader))
    ...
    def readXML(self, typ):
        if typ == "l":
            ...
            self.showLog = False
            part = None
        # returns True/False and data or error
            ...
            if not self.showLog:
                ...
                for client in clients:
                    ...
                    if srvname is not None:
                        srvname_short = srvname.split(
                            ":")[1].strip() if ":" in srvname else srvname
                    ...
                    if name != "" and name != "anonymous" and proto != "":
                        try:
                            tmp[client.attrib["type"]].append((name, proto, "%s:%s" % (
                                caid, srvid), srvname_short, ecmtime, ip, connstatus))
                        except KeyError:
                    ...
                    if client.attrib["type"] == "p" or client.attrib["type"] == "r":
                        if spec is not None:
                            ...
                            if client.attrib["name"] != "" and client.attrib["name"] != "" and client.attrib["protocol"] != "":
                                # return tuple for later use in Choicebox
                                readers.append(
                                    (client.attrib["name"], client.attrib["name"]))
            return readers
        else:
            return None
        ...
        if NAMEBIN:
            ...
            if entry in (
                ...
                if ospath.exists("/tmp/ecm.info"):
                    ...
                    self.session.open(
                        MessageBox,
                        _("No ECM info is currently available. This is only available while decrypting."),
                ...
                osc = OscamInfo()
                # get list of available CCcam-Readers
                ...
                if isinstance(reader, list):
                    if len(reader) == 1:
                        ...
                        self.session.openWithCallback(
                            self.chooseReaderCallback,
                            ChoiceBox,
                            title=_("Choose CCcam-Reader"),
                            list=reader)
            elif entry == 5:
                osc = OscamInfo()
                ...
                if reader is not None:
                    ...
                    if isinstance(reader, list):
                        if len(reader) == 1:
                            ...
                            self.session.openWithCallback(
                                self.chooseReaderCallback, ChoiceBox, title=_("Choose reader"), list=reader)
            elif entry == 6:
                self.session.open(OscamInfoConfigScreen)
        else:
            self.session.open(
                MessageBox,
                _("Oscam not running - start Cam to obtain information."),
                MessageBox.TYPE_INFO)
            ...
            if t.startswith("--"):
                ...
                if png is not None:
                    x, y, w, h = skin.parameters.get(
                        "ChoicelistDash", (0, 2 * f, 800 * f, 2 * f))
                    res.append(
                        (eListboxPythonMultiContent.TYPE_PIXMAP, x, y, w, h, png))
                    x, y, w, h = skin.parameters.get(
                        "ChoicelistName", (45 * f, 2 * f, 800 * f, 25 * f))
                    res.append((eListboxPythonMultiContent.TYPE_TEXT,
                               x, y, w, h, 0, RT_HALIGN_LEFT, t[2:]))
                    png2 = resolveFilename(
                        SCOPE_CURRENT_SKIN, "buttons/key_" + keys[k] + ".png")
                    if fileExists(png2):
                        png2 = LoadPixmap(png2)
                    if png2 is not None:
                        x, y, w, h = skin.parameters.get(
                            "ChoicelistIcon", (5 * f, 0, 35 * f, 25 * f))
                        res.append(
                            (eListboxPythonMultiContent.TYPE_PIXMAP_ALPHABLEND, x, y, w, h, png2))
            else:
                x, y, w, h = skin.parameters.get(
                    "ChoicelistName", (45 * f, 2 * f, 800 * f, 25 * f))
                res.append((eListboxPythonMultiContent.TYPE_TEXT,
                           x, y, w, h, 0, RT_HALIGN_LEFT, t))
                png2 = resolveFilename(
                    SCOPE_CURRENT_SKIN,
                    "buttons/key_" + keys[k] + ".png")
                if fileExists(png2):
                    png2 = LoadPixmap(png2)
                if png2 is not None:
                    x, y, w, h = skin.parameters.get(
...
class oscInfo(Screen, OscamInfo):
    ...
    def __init__(self, session, what):
        ...
        self.skin = """<screen position="center,center" size="%d, %d" title="Client Info" >""" % (
            sizeH, ysize)
        button_width = int(sizeH / 4)
        for k, v in enumerate(["red", "green", "yellow", "blue"]):
            xpos = k * button_width
            self.skin += """<ePixmap name="%s" position="%d,%d" size="35,25" pixmap="buttons/key_%s.png" zPosition="1" transparent="1" alphatest="on" />""" % (
                v, xpos, ypos, v)
            self.skin += """<widget source="key_%s" render="Label" position="%d,%d" size="%d,%d" font="Regular;18" zPosition="1" valign="center" transparent="1" />""" % (
                v, xpos + 40, ypos, button_width, 22)
        self.skin += """<ePixmap name="divh" position="center,37" size="%d,%d" pixmap="div-h.png" transparent="1" alphatest="blend" scale="1" zposition="2" />""" % (
            sizeH, int(2 * f))
        self.skin += """<widget name="output" position="10,45" size="%d,%d" zPosition="1" scrollbarMode="showOnDemand" />""" % (
            self.sizeLH, ysize - 50)
        self.skin += """</screen>"""
        Screen.__init__(self, session)
    ...
    def buildListEntry(self, listentry, heading=False):
        ...
        if isinstance(self.errmsg, tuple):
            # overrides previous font-size in case of an error message. (if
            # self.errmsg is a tuple, an error occurred which will be displayed
            # instead of regular results
            useFont = 0
        elif heading:
            useFont = 1
            ypos = -2
        ...
        if heading:
            ...
            if png is not None:
                res.append(
                    (eListboxPythonMultiContent.TYPE_PIXMAP,
                     0,
                     (self.itemheight - 2) * f,
                        self.sizeLH,
                        2 * f,
    ...
    def showData(self):
        ...
        if self.listchange:
            ...
            self.rows = int(self["output"].instance.size(
            ).height() / (self.itemheight * f))
...
class oscEntitlements(Screen, OscamInfo):
    ...
    def buildList(self, data):
        ...
        for i in caids:
            ...
            res.append((ca_id, csystem, str(hops[1]), str(hops[2]), str(hops[3]), str(
                hops[4]), str(hops[5]), str(csum), str(creshare), providertxt[:-1]))
            outlist.append(res)
        return res

    def showData(self):
        dataWebif_for_reader = self.openWebIF(
            part="entitlement", reader=self.cccamreader)
        ...
        for i in cards:
            ...
            if ccaid in caid:
                ...
                for prov in provs.findall("provider"):
                    caid[ccaid]["provider"].append(
                        (prov.attrib["provid"], prov.text))
    ...
    def showData(self):
        ...
        for i in readers:
            ...
            if dataWebif[0]:
                ...
                ecmcount = ecmstat.attrib["count"] and int(
                    ecmstat.attrib["count"]) or 0
                ...
                if ecmcount > 0:
                    for j in ecm:
                        ...
                        if rcs == "found":
                            ...
                            if "lastrequest" in j.attrib:
                                ...
                                try:
                                    ...
                                    last_req = time.strftime(
                                        "%H:%M:%S", time.localtime(float(lastreq)))
                            else:
                                last_req = ""
                        else:
                        ...
                        if self.allreaders:
                            result.append(
                                (i[1], caid, channel, avg_time, last_time, rcs, last_req, int(num)))
...
class OscamInfoConfigScreen(ConfigListScreen, Screen):
    ...
    def __init__(self, session, msg=None):
        ...
        self["status"] = StaticText(
            _("Error:\n%s") %
            msg if msg is not None else "")  # what is this?
        ConfigListScreen.__init__(
            self, [], session=session, on_change=self.changedEntry)
        self["actions"] = ActionMap(
            ["SetupActions"],
            {
                "ok": self.savx,
    ...
    def changedEntry(self):
        if self["config"].getCurrent() and len(
                self["config"].getCurrent()) > 1 and self["config"].getCurrent()[1] in (
    ...
    def createSetup(self):
        oscamconfig = [(getConfigListEntry(_("Read Userdata from %s.conf" %
                        check_NAMEBIN()), config.oscaminfo.userdatafromconf))]
        ...
        oscamconfig.append(
            getConfigListEntry(
                _("Automatically update Client/Server View?"),
                config.oscaminfo.autoupdate))
        if config.oscaminfo.autoupdate.value:
            oscamconfig.append(
                getConfigListEntry(
                    _("Update interval (in seconds)"),
