...
(e.g., TMDb, OMDb, TheTVDB, Fanart) from skin files in the Enigma2 environment.
...
for api, api_config in API_CONFIG.items():
    skin_path = f"{mvi}enigma2/{cur_skin}/{api_config['skin_file']}"
    globals()[f"{api}_skin"] = skin_path
    globals()[api_config['var_name']] = api_config['default_key']


def load_api_keys():
    """Loads API keys from skin files or falls back to defaults.
    Args:
        api_config_map (dict): Configuration mapping for each API.
        skin_name (str): Current skin folder name.
        base_path (str): Root path to skin folders.
        verbose (bool): If True, prints status for each key.
    """
    global my_cur_skin
    if my_cur_skin:
        return
    ...
    try:
        skin_paths = {
            api: f"/usr/share/enigma2/{cur_skin}/{config['skin_file']}"
            for api, config in API_CONFIG.items()
        }

        for api, api_config in API_CONFIG.items():
            file_path = skin_paths[api]
            ...
            if key_value:
                globals()[config['var_name']] = key_value
...
config.plugins.Aglare.tmdb = NoSave(ConfigOnOff(default=False))
config.plugins.Aglare.load_tmdb_api = NoSave(ConfigYesNo(default=False))  # NoSave(ConfigSelection(['-> Ok']))
config.plugins.Aglare.tmdb_api = ConfigText(default=tmdb_api, visible_width=50, fixed_size=False)

config.plugins.Aglare.omdb = NoSave(ConfigOnOff(default=False))
config.plugins.Aglare.load_omdb_api = NoSave(ConfigYesNo(default=False))  # NoSave(ConfigSelection(['-> Ok']))
config.plugins.Aglare.omdb_api = ConfigText(default=omdb_api, visible_width=50, fixed_size=False)

config.plugins.Aglare.thetvdb = NoSave(ConfigOnOff(default=False))
config.plugins.Aglare.load_thetvdb_api = NoSave(ConfigYesNo(default=False))  # NoSave(ConfigSelection(['-> Ok']))
config.plugins.Aglare.thetvdb_api = ConfigText(default=omdb_api, visible_width=50, fixed_size=False)

config.plugins.Aglare.fanart = NoSave(ConfigOnOff(default=False))
config.plugins.Aglare.load_fanart_api = NoSave(ConfigYesNo(default=False))  # NoSave(ConfigSelection(['-> Ok']))
config.plugins.Aglare.fanart_api = ConfigText(default=omdb_api, visible_width=50, fixed_size=False)
...
class AglareSetup(ConfigListScreen, Screen):
    ...
    def createSetup(self):
        try:
            ...
            if config.plugins.Aglare.actapi.value is True:
                list.append(getConfigListEntry("TMDB API:", config.plugins.Aglare.tmdb, _("Settings TMDB ApiKey")))
                if config.plugins.Aglare.tmdb.value is True:
                    list.append(getConfigListEntry("--Load TMDB Apikey", config.plugins.Aglare.load_tmdb_api, _("Load TMDB Apikey from /tmp/tmdbkey.txt")))
                ...
                list.append(getConfigListEntry("OMDB API:", config.plugins.Aglare.omdb, _("Settings OMDB ApiKey")))
                if config.plugins.Aglare.omdb.value is True:
                    list.append(getConfigListEntry("--Load OMDB Apikey", config.plugins.Aglare.load_omdb_api, _("Load OMDB Apikey from /tmp/omdbkey.txt")))
                ...
                list.append(getConfigListEntry("THETVDB API:", config.plugins.Aglare.thetvdb, _("Settings THETVDB ApiKey")))
                if config.plugins.Aglare.thetvdb.value is True:
                    list.append(getConfigListEntry("--Load THETVDB Apikey", config.plugins.Aglare.load_thetvdb_api, _("Load THETVDB Apikey from /tmp/thetvdbkey.txt")))
                    list.append(getConfigListEntry("--Set THETVDB Apikey", config.plugins.Aglare.thetvdb_api, _("Signup on THETVDB and input free personal ApiKey")))

                list.append(getConfigListEntry("FANART API:", config.plugins.Aglare.fanart, _("Settings FANART ApiKey")))
                if config.plugins.Aglare.fanart.value is True:
                    list.append(getConfigListEntry("--Load FANART Apikey", config.plugins.Aglare.load_fanart_api, _("Load FANART Apikey from /tmp/fanartkey.txt")))
                    list.append(getConfigListEntry("--Set FANART Apikey", config.plugins.Aglare.fanart_api, _("Signup on FANART and input free personal ApiKey")))
    ...
    def keyRun(self):
        ...
        action_map = {
            ...
            config.plugins.Aglare.load_tmdb_api: self.load_tmdb_api,
            config.plugins.Aglare.tmdb_api: self.KeyText,
            config.plugins.Aglare.load_omdb_api: self.load_omdb_api,
            config.plugins.Aglare.omdb_api: self.KeyText,
            config.plugins.Aglare.load_thetvdb_api: self.load_thetvdb_api,
            config.plugins.Aglare.thetvdb_api: self.KeyText,
            config.plugins.Aglare.load_fanart_api: self.load_fanart_api,
            config.plugins.Aglare.fanart_api: self.KeyText,
        }
    ...
    def handle_api_load(self, api_name, api_file, config_entry, skin_file, answer=None):
        if answer is None:
            if fileExists(api_file) and stat(api_file).st_size > 0:
                self.session.openWithCallback(
                    lambda answer: self.handle_api_load(api_name, api_file, config_entry, skin_file, answer),
                    MessageBox,
                    _("Import Api Key %s from %s?") % (api_name, api_file)
                )
            else:
                self.session.open(MessageBox, _("File %s missing!") % api_file, MessageBox.TYPE_INFO, timeout=4)
        elif answer:
            if fileExists(api_file) and stat(api_file).st_size > 0:
                ...
                if not fpage:
                    self.session.open(MessageBox, _("%s ApiKey vuota!") % api_name, MessageBox.TYPE_INFO, timeout=4)
                    return

                with open(skin_file, "w") as t:
                    t.write(fpage)

                config_entry.setValue(fpage)
                config_entry.save()
                self.session.open(MessageBox, _("%s ApiKey importata!") % api_name, MessageBox.TYPE_INFO, timeout=4)
            else:
                self.session.open(MessageBox, _("File %s non trovato!") % api_file, MessageBox.TYPE_INFO, timeout=4)

        self.createSetup()

    def load_tmdb_api(self, answer=None):
        self.handle_api_load("TMDB", "/tmp/tmdbkey.txt", config.plugins.Aglare.tmdb_api, tmdb_skin, answer)

    def load_omdb_api(self, answer=None):
        self.handle_api_load("OMDB", "/tmp/omdbkey.txt", config.plugins.Aglare.omdb_api, omdb_skin, answer)

    def load_thetvdb_api(self, answer=None):
        self.handle_api_load("THETVDB", "/tmp/thetvdbkey.txt", config.plugins.Aglare.thetvdb_api, thetvdb_skin, answer)

    def load_fanart_api(self, answer=None):
        self.handle_api_load("FANART", "/tmp/fanartkey.txt", config.plugins.Aglare.fanart_api, fanart_skin, answer)
    ...
    def handleKeyActions(self):
        ...
        reset_map = {
            ...
            config.plugins.Aglare.load_tmdb_api: (config.plugins.Aglare.load_tmdb_api, self.load_tmdb_api),
            config.plugins.Aglare.load_omdb_api: (config.plugins.Aglare.load_omdb_api, self.load_omdb_api),
            config.plugins.Aglare.load_thetvdb_api: (config.plugins.Aglare.load_thetvdb_api, self.load_thetvdb_api),
            config.plugins.Aglare.load_fanart_api: (config.plugins.Aglare.load_fanart_api, self.load_fanart_api),
        }
