...
class AglareCaidInfo2(Poll, Converter, object):
    ...
    def __init__(self, type):
        ...
        self.eDVBCIUIInstance and self.eDVBCIUIInstance.ciStateChanged.get().append(
            self.ciModuleStateChanged)
        self.NUM_CI = eDVBCIInterfaces.getInstance(
        ) and eDVBCIInterfaces.getInstance().getNumOfSlots()
        if DBG:
            j00zekDEBUG(
                '[j00zekModCaidInfo2:__init__] self.NUM_CI = %s' % str(
                    self.NUM_CI))
        if type == "CAID":
            self.type = self.CAID
        elif type == "PID":
                ...
                if self.type == self.IS_NET:
                    if using == "CCcam":
                        ...
                        if source != "cache" and source == "net" and source.find(
                                "emu") == -1:
                            return True
                            # return  (source != None and source == "net") or
                            # (source != None and source != "sci") or (source
                            # != None and source != "emu") or (reader != None
                            # and reader != "emu") or (source != None and
                            # source != "card")
                else:
                    return False
    ...
    def runningSoftCamName(self, fullName=False):
        ...
        try:
            procStat = open(join("/proc", str(self.currPID), 'stat'),
            ...
            if DBG:
                j00zekDEBUG('\t Exception trying to get name of curr.PID= %s : %s' % (
                    join("/proc", str(self.currPID)), str(e)))
        ...
        for f in listdir("/proc"):
            if isdir(join("/proc", f)):
                try:
                    ...
                    if DBG:
                        j00zekDEBUG(
                            '\t Exception trying to analyze %s : %s' %
                            (join(
                                "/proc", f), str(e)))
    ...
    def getText(self):
        ...
        softCamName = ''
        service = self.source.service
        # name softcam name is independent, so here to always return it when
        # requested
    ...
    def ecmfileContent(self):
        ...
        try:
            if fileExists("/tmp/ecm.info"):
                with open("/tmp/ecm.info", "r") as ecmfiles:
                    for line in ecmfiles:
                        if line.find("caid:") > -1 or line.find("provider:") > -1 or line.find("provid:") > -1 or line.find("pid:") > -1 or line.find(
                                "hops:") > -1 or line.find("system:") > -1 or line.find("address:") > -1 or line.find("using:") > -1 or line.find("ecm time:") > -1:
                            line = line.replace(' ', "").replace(":", ": ")
                        if line.find("caid:") > -1 or line.find("pid:") > -1 or line.find("reader:") > -1 or line.find("from:") > -1 or line.find(
                                "hops:") > -1 or line.find("system:") > -1 or line.find("Service:") > -1 or line.find("CAID:") > -1 or line.find("Provider:") > -1:
                            line = line.strip('\n') + "  "
                        ...
                        if line.find("response time:") > -1:
                            line = line.replace(
                                "response time:", "ecm time:").replace(
                                "decoded by", "by")
            ...
            if DBG:
                j00zekDEBUG(
                    '\t Exception analyzing content of /tmp/ecm.info : %s' %
                    str(e))
        return ecminfo
                        ...
                        if len(item) > 1:
                            ...
                            if item[0] == "Provider":
                                ...
                                if y != -1:
                                    ...
                                    item[0] = "source"
                                    item[1] = "sci"
                                if it_tmp[-1].find('emu') > -1 or it_tmp[-1].find('EMU') > -1 or it_tmp[-1].find(
